...
- Client `INSERT`/`UPDATE`/`DELETE` grants are **revoked** on every table, so there is
  no direct client write path. This is what enforces "clients cannot mint seconds" and
  keeps the server-managed profile flags tamper-proof at the DB layer.
...
After any schema change, regenerate the frontend types:
...
- [x] Live Supabase instance mirrors the documented schema (5 tables, all RLS enabled).
- [x] `users` connected to `auth.users`; account graph auto-provisioned on signup via trigger.
- [x] Content ingestion, gamification currency (seconds), and quiz metrics modeled.
...
- [x] Verified: provisioning, read isolation, write denial, cascade, 0 security advisor lints.
